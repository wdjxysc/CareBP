...
+(NSDictionary *)uploadTemperatureData:(NSDictionary *)datadic authkey:(NSString *)authkey
{
    ...
    NSString *url = [[NSString alloc]initWithFormat:@"http://test.ebelter.com/service/ehealth_uploadEarThermometerData?authkey=%@&temperature=%@&time=%@&showresult=true&showadvice=false",
                     authkey,
                     [[NSString alloc] initWithFormat:@"%.1f",temperature],
                     testtimestr];
    dic = [self getDictionaryByUrl:url];
    
    NSString *urladvice = [[NSString alloc]initWithFormat:@"http://test.ebelter.com/service/ehealth_uploadEarThermometerData?authkey=%@&temperature=%@&time=%@&showresult=true&showadvice=true",
                     authkey,
                     [[NSString alloc] initWithFormat:@"%.1f",temperature],
                     testtimestr];
...
+(NSDictionary *)uploadWeightData:(NSDictionary *)datadic authkey:(NSString *)authkey
{
    ...
    NSString *url = [[NSString alloc]initWithFormat:@"http://test.ebelter.com/service/ehealth_uploadWeightData?authkey=%@&weight=%.1f&height=%d&time=%@&showresult=true&showadvice=false",
                     authkey,
                     weight,
                     height,
                     testtimestr];
    ...
    NSString *urlgetadvice = [[NSString alloc]initWithFormat:@"http://test.ebelter.com/service/ehealth_uploadWeightData?authkey=%@&weight=%.1f&height=%d&time=%@&showresult=true&showadvice=true",
                     authkey,
                     weight,
                     height,
                     testtimestr];
...
+(NSDictionary *)uploadBodyFatData:(NSDictionary *)datadic authkey:(NSString *)authkey
{
    ...
    NSString *url = [[NSString alloc]initWithFormat:@"http://test.ebelter.com/service/ehealth_uploadBodyCompositionData?authkey=%@&muscle=%.1f&adiposerate=%.1f&Metabolism=%d&visceralfat=%.1f&moisture=%.1f&bone=%.1f&thermal=%d&impedance=%d&bmi=%.1f&time=%@&showtresult=true&showadvice=false&shareid=123",
                     authkey,
                     muscle,
                     adiposerate,
                     thermal,
    ...
    NSString *urlgetadvice = [[NSString alloc]initWithFormat:@"http://test.ebelter.com/service/ehealth_uploadBodyCompositionData?authkey=%@&muscle=%.1f&adiposerate=%.1f&Metabolism=%d&visceralfat=%.1f&moisture=%.1f&bone=%.1f&thermal=%d&impedance=%d&bmi=%.1f&time=%@&showtresult=true&showadvice=true&shareid=123",
                     authkey,
                     muscle,
                     adiposerate,
                     thermal,
...
+(NSDictionary *)uploadBloodPressureData:(NSDictionary *)datadic authkey:(NSString *)authkey
{
    ...
    NSString *url = [[NSString alloc]initWithFormat:@"http://test.ebelter.com/service/ehealth_uploadBloodPressureData?authkey=%@&systolic=%d&diastolic=%d&pulse=%d&time=%@&shareid=123&showresult=true&showadvice=false",
                     authkey,
                     sys,
                     dia,
                     pulse,
    ...
    NSString *urladvice = [[NSString alloc]initWithFormat:@"http://test.ebelter.com/service/ehealth_uploadBloodPressureData?authkey=%@&systolic=%d&diastolic=%d&pulse=%d&time=%@&shareid=123&showresult=true&showadvice=true",
                     authkey,
                     sys,
                     dia,
                     pulse,
...
+(NSString *)registByEmail:(NSString *)email password:(NSString *)password
{
    ...
    NSString *url = [[NSString alloc] initWithFormat:
                     @"http://test.ebelter.com/public/right_emailRegister?user.email=%@&user.password=%@&userInfo.sex=0&userInfo.birthday=1990-01-01&userInfo.height=175&userInfo.weight=65&userInfo.profession=1",email,password];
...
+(NSDictionary *)getCheckCodeByEmail:(NSString *)email
{
    ...
    NSString *url = [[NSString alloc] initWithFormat:@"http://test.ebelter.com/service/ehealth_checkTelphone?dtype=30&telphone=%@&flag=true",email];
    dic = [self getDictionaryByUrl:url];
...
+(NSDictionary *)checkCheckCode:(NSString *)email checkcode:(NSString *)checkcode
{
    ...
    NSString *url = [[NSString alloc] initWithFormat:@"http://test.ebelter.com/service/ehealth_checkTelphone?dtype=30&telphone=%@&&flag=true&&check_code=%@",email,checkcode];
    dic = [self getDictionaryByUrl:url];
...
+(NSDictionary *)resetPasswordByEmail:(NSString *)email newpassword:(NSString *)newpassword
{
    ...
    NSString *url = [[NSString alloc] initWithFormat:@"http://test.ebelter.com/service/ehealth_resetPassword?telphone=%@&dtype=30&newpassword=%@",email,newpassword];
    dic = [self getDictionaryByUrl:url];
...
+(NSArray *)deleteProductAttention :(NSString *)attention_id
{
    ...
    NSString *url = [[NSString alloc]initWithFormat:@"http://test.ebelter.com/sys/productManage_deleteProductAttention?productAttentionVo.attention_id=%@",attention_id];
    array = [self getArrayByUrl:url];
    return array;
}
...
+(NSDictionary *)terminalGetContent :(int)apptypeid number:(int)number
{
    NSDictionary *dic;
    
    //http://192.168.110.36:8080/sys/appBlood_terminalGetContent?appTypeId=2&number=4
    NSString *url = [[NSString alloc]initWithFormat:@"http://192.168.110.36:8080/sys/appBlood_terminalGetContent?appTypeId=%d&number=%d",apptypeid,number];
    dic = [self getDictionaryByUrl:url];
    return dic;
}
...
+(NSDictionary *)terminalGetQuestion :(int)apptypeid
{
    ...
    NSString *url = [[NSString alloc]initWithFormat:@"http://192.168.110.36:8080/sys/appBlood_terminalGetQuestion?appTypeId=%d",apptypeid];
    dic = [self getDictionaryByUrl:url];
    return dic;
}
...
+(NSDictionary *)terminalGetAnswer : (int)questionid
{
    ...
    NSString *url = [[NSString alloc]initWithFormat:@"http://192.168.110.36:8080/sys/appBlood_terminalGetAnswer?questionId=%d",questionid];
    dic = [self getDictionaryByUrl:url];
    return dic;
}
